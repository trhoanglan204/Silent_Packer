...
int find_pe_code_cave_index(t_pe* pe) {
    ...
    if (pe->s_type == PE32) {
        ...
        //    if (strncmp(section_name, ".data", 5) == 0 || strncmp(section_name, ".text", 5) == 0) {
...
int pe_cave_insert_loader(t_pe* pe, int section_index, int old_section_size) {
    ...
    if (pe->s_type == PE32) {
        new_section_data = (char*)realloc(((t_pe32*)pe)->section_data[section_index], old_section_size + packer_config.loader_size);
        ...
        new_section_data = (char*)realloc(((t_pe64*)pe)->section_data[section_index], old_section_size + packer_config.loader_size);
        if (new_section_data == NULL) {
            log_error("realloc() failure");
            return -1;
        }
